import { Bell, ChevronRight, LogOut, Phone, Shield, User, Users } from 'lucide-react';
import { useCallback, useMemo, useRef, useState } from 'react';

import { RECENT_ALERTS } from '../../data/recentAlerts.js';
import { useOnClickOutside } from '../../hooks/useOnClickOutside.js';

export function TopNav({ onNavigate, isCrowdShieldActive, onToggleCrowdShield, onTriggerFakeCall }) {
  ...
  useOnClickOutside(notifRef, closeNotifications, showNotifications);
  useOnClickOutside(profileRef, closeProfile, showProfileMenu);

  const onLogoClick = useCallback(() => onNavigate('dashboard'), [onNavigate]);

  const crowdShieldBtnClass = useMemo(() => {
    return isCrowdShieldActive
    ...
    <header className="h-20 flex items-center justify-between px-4 sm:px-6 lg:px-10 sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200 shadow-sm shrink-0">
      ...
      <div className="flex items-center gap-3 sm:gap-5">
        <button
          ...
          className={`hidden md:flex items-center gap-2 px-4 py-2 rounded-full border font-bold text-[13px] uppercase tracking-wider transition-colors ${crowdShieldBtnClass}`}
        >
          <span
